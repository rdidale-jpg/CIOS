...
from cios.applications.opportunity_assistant.rules import RuleMatch, evaluate_rules
...
def run_pipeline(path: Path = SAMPLE_PATH) -> dict[str, Any]:
    ...
    rule_matches = evaluate_rules(source)
...
def _create_ontology(source: dict[str, Any]) -> dict[str, Any]:
    customer = Customer(
...
def _create_graph(ontology: dict[str, Any], evidence: list[Evidence]) -> KnowledgeGraphRecord:
...
def _create_reasoning(rule_matches: list[RuleMatch], observations: list[Observation]) -> dict[str, Any]:
    ...
    return {"signals": signals, "hypotheses": hypotheses, "inferences": inferences, "explanations": [explanation], "trace": trace, "result": ReasoningResult(summary=explanation.summary, trace=trace, hypotheses=hypotheses, signals=signals, inferences=inferences, explanations=[explanation])}


def _create_scoring(rule_matches: list[RuleMatch], trace: ReasoningTrace, evidence: list[Evidence]) -> dict[str, Any]:
    ...
    return {"result": result, "transformation_pressure": TransformationPressureScore(result=result, urgency_score=components[4].score, strategic_importance_score=components[0].score, change_pressure_score=components[1].score, capability_gap_score=components[2].score)}


def _create_decision(source: dict[str, Any], graph: KnowledgeGraphRecord, evidence: list[Evidence], observations: list[Observation], reasoning: dict[str, Any], scoring: dict[str, Any]) -> DecisionOutput:
