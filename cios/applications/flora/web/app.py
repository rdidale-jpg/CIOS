...
class FloraWebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - stdlib callback name
        ...
        try:
            ...
            if parsed.path == "/health":
                ...
                self._html(html, status=status)
            elif parsed.path.startswith("/blueprint-import/") and "/aspects/" in parsed.path:
                remainder = parsed.path.removeprefix("/blueprint-import/")
                run_id, aspect = remainder.split("/aspects/", 1)
                domain = (parse_qs(parsed.query).get("domain") or ["all"])[0]
                html, status = executive_workspace_page(run_id, self.headers, view="aspect", collection=aspect, domain=domain)
                self._html(html, status=status)
                ...
                html, status = executive_workspace_page(run_id, self.headers, view="enterprise", enterprise_id=enterprise_id)
                self._html(html, status=status)
                ...
                self._html(html, status=status)
            elif parsed.path.startswith("/blueprint-import/") and parsed.path.endswith("/delete"):
                run_id = parsed.path.removeprefix("/blueprint-import/").removesuffix("/delete")
                html, status = blueprint_delete_candidate_preview_page(run_id, self.headers)
                self._html(html, status=status)
            elif parsed.path.startswith("/blueprint-import/") and parsed.path.endswith("/archive"):
                run_id = parsed.path.removeprefix("/blueprint-import/").removesuffix("/archive")
                html, status = blueprint_archive_release_page(run_id, self.headers)
                self._html(html, status=status)
    ...
    def do_POST(self) -> None:  # noqa: N802 - stdlib callback name
        ...
        if self.path == "/pilot-sign-in":
            ...
            self._html(html, status=status)
        elif self.path.startswith("/blueprint-import/") and self.path.endswith("/delete"):
            run_id = self.path.removeprefix("/blueprint-import/").removesuffix("/delete")
            html, status = blueprint_delete_candidate_import(run_id, form, self.headers)
            self._html(html, status=status)
...
def _flora_home_page(headers=None, question_error: str = "") -> str:
    ...
    def opportunity_card(o):
        ...
        return (f"<article class='priority-card'>{hypothesis}<h3>{customer}</h3><h4>{escape(o.title)}</h4>"
                ...
                "<div><dt>Timing</dt><dd>Not supplied</dd></div><div><dt>Value</dt><dd>Not supplied</dd></div>"
    ...
    return _flora_v2_page("Enterprise Intelligence Workspace", "home", f"""
      ...
      <section aria-labelledby='map-title'><div class='section-heading'><div><p class='eyebrow'>Flora Map</p><h2 id='map-title'>Enterprise Intelligence Map</h2></div><p>Choose a domain to inspect its current intelligence and next decision.</p></div><div class='estate-grid'>
        ...
        {domain('/intelligence?tab=programmes','Major Programmes','Not currently summarised','Programme summary not available.','Open programmes')}
...
def _flora_opportunities_page(headers=None, query=None) -> str:
    ...
    cards = "".join(f"<article class='priority-card'><p class='eyebrow'>{escape(_horizon_label(o.horizon))}</p><h2>{escape(o.title)}</h2><p><strong>Customer</strong> {escape(o.enterprise_name or o.participant_type)}</p><p><strong>Problem</strong> {escape(o.commercial_problem)}</p><dl><div><dt>Timing</dt><dd>Not supplied</dd></div><div><dt>Value</dt><dd>Not supplied</dd></div><div><dt>Confidence</dt><dd>{escape(o.confidence)}</dd></div><div><dt>Evidence</dt><dd>{escape(o.evidence_strength)}</dd></div></dl><p><strong>Reason for Horizon</strong> {escape(o.horizon_rationale.rationale)}</p><a href='/focus?opportunity={escape(o.opportunity_id)}'>Inspect evidence and rationale</a></article>" for o in selected)
    if requested not in {"horizon_1", "horizon_2", "horizon_3"}: cards = "<section class='card'><h2>Summary not available</h2><p>The current runtime does not provide a separate governed summary for this view. Existing opportunity intelligence remains available in the Horizon views.</p><a href='/focus'>Open the full opportunity pipeline</a></section>"
