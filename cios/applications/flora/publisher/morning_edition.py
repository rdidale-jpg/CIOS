"""Generate Flora Executive Brief PDF and HTML publications."""
...
def build_publication_context(publication_date: date | None = None) -> dict[str, Any]:
    ...
    movers = sorted(top_five, key=lambda i: score_adjustments[i.organisation].live_evidence_score + score_adjustments[i.organisation].learned_evidence_score, reverse=True)
    if live_metrics:
        change_items = []
    ...
    commercial_pipeline = {
        ...
        "highest_conviction_arguments": [vars(a) for a in sorted(observatory.commercial_arguments, key=lambda a: a.confidence, reverse=True)[:5]],
        "recommended_executive_conversations": [vars(r) for r in observatory.executive_recommendations[:5]],
...
def render_markdown(ctx: dict[str, Any]) -> str:
    lines = [f"# Flora Executive Brief — {ctx['publication_date']}", "", "**Confidential pilot preview.**", "", f"**Publication date:** {ctx['publication_date_label']}", f"**Version:** {ctx['version']}", f"**Reading time:** {ctx['reading_time']} minutes", f"**Generated:** {ctx['generated_timestamp']}", "", f"## {ctx['live_evidence_banner']}"]
    summary = ctx["live_coverage_summary"]
    radar = ctx.get("portfolio_radar_summary", {})
    lines += ["", "### Evidence status", "### Live Evidence Coverage", f"- {summary['sources_attempted']} sources attempted", f"- {summary['sources_succeeded']} succeeded", f"- {summary['sources_produced_evidence']} produced evidence", f"- {summary['evidence_objects']} evidence objects", f"- Commercial-sector coverage: {summary.get('commercial_sector_coverage', 0)}", f"- Public-sector coverage: {summary.get('public_sector_coverage', 0)}", f"- Organisations configured: {summary.get('organisations_configured', 0)}", f"- Organisations covered: {summary.get('organisations_covered', 0)}", f"- Organisations newly covered: {', '.join(summary.get('organisations_newly_covered', [])) or 'None'}", f"- Failed source count: {summary.get('failed_source_count', 0)}", f"- Source coverage health: {summary.get('source_coverage_health', 'unknown')}", f"- Highest-quality sources used: {summary.get('highest_quality_sources', [])}", f"- Uncovered priority organisations: {', '.join(summary.get('uncovered_priority_organisations', [])) or 'None'}", f"- Live evidence by sector: {summary.get('live_evidence_by_sector', {})}", f"- Strongest covered organisations: {', '.join(summary['strongest_covered_organisations']) or 'None'}", f"- Top uncovered organisations: {', '.join(summary.get('top_uncovered_organisations', [])) or 'None'}", f"- Uncovered organisations: {', '.join(summary['uncovered_organisations']) or 'None'}"]
    lines += ["", "### Portfolio Radar Summary", f"- Priority Pursuits: {radar.get('priority_pursuits', 0)}", f"- Investigate organisations: {radar.get('investigate', 0)}", f"- Monitor organisations: {radar.get('monitor', 0)}", f"- Coverage Gaps: {radar.get('coverage_gaps', 0)}", f"- Most under-covered high-potential organisation: {radar.get('most_under_covered_high_potential', 'None')}", f"- Strongest evidence-backed mover: {radar.get('strongest_evidence_backed_mover', 'None')}", f"- Best performing source: {radar.get('best_performing_source', 'None')}", f"- Weakest source category: {radar.get('weakest_source_category', 'None')}"]
    cp = ctx.get("commercial_pipeline", {})
    lines += ["", "## Commercial Intelligence Pipeline", "", "### Top Commercial Signals"]
    lines += [f"- **{s['signal_id']}** — {s['title']}: {s['observation']} Supports: {', '.join(s['supports'])}. Does not support: {', '.join(s['does_not_support'])}." for s in cp.get("top_commercial_signals", [])]
    lines += ["", "### Top Emerging Insights"] + [f"- **{i['insight_id']}** — {i['summary']} ({i['hypothesis_type']}; confidence {i['confidence']})" for i in cp.get("top_emerging_insights", [])]
    ...
    for k, v in ctx["priority_opportunity"].items():
        lines += ["", f"### {k}", str(v)]
    lines += ["", "## Next best conversations", "", "| Organisation | Sector | Condition Strength | Base Score | Live Uplift | Final Score | Live Evidence | Unique Sources | Strongest Live Condition | Strongest Live Capability | Movement | Confidence |", "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- | ---: |"]
...
def main() -> None:
    ...
    print(f"Flora Executive Brief generated: {paths['pdf'].resolve()}")
    print("Flora Morning Edition generated (compatibility label)")
