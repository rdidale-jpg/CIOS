...
def landing_page() -> str:
    ...
    <section class='card action'><h2>So What? / Next Best Conversation</h2><p>What changed, why it matters, who to speak to, what to validate, and what not to overclaim.</p><ol>{"".join(f"<li><strong>{escape(a['organisation'])}</strong> ({escape(a['time_required'])}, {escape(a['target_executive_or_function'])}): {escape(a['action'])} Proposition: {escape(a['proposition'])}. Missing: {escape(', '.join(a['missing_evidence']))}</li>" for a in pub["recommended_actions"][:3])}</ol></section>
...
def settings_page() -> str:
    ...
    return _page("Settings", f"<section class='hero'><h1>Settings</h1><p>Provider context is configurable. This local pilot renders default IBM context; fields are shown read-only for deterministic local execution.</p></section><section class='card'><h2>Provider Profile</h2><span hidden>Provider Context</span><table>{rows}</table></section><section class='card'><h2>Local edit preview</h2><form>{form}<p><button type='button'>Local editing placeholder</button></p></form></section>")
...
def score_page(slug: str) -> str:
    ...
    body = f"""<section class='hero'><h1>{escape(detail['organisation'])} score explainability</h1><p>{escape(detail['sector'])} · quadrant: <strong>{escape(detail['quadrant'])}</strong> · scoring mode: <strong>{escape(detail.get('scoring_mode_display', detail['live_scoring_mode']))}</strong></p><div class='grid'><div><div class='metric'>{detail['final_score']}</div><p>final score</p></div><div><div class='metric'>{detail['total_platform_live_evidence_count']}</div><p><a href='/live/evidence'>total platform live evidence objects</a></p></div><div><div class='metric'>{detail['unique_evidence_count']}</div><p><a href='/radar'>organisation live evidence objects</a></p></div><div><div class='metric'>{detail['unique_source_count']}</div><p><a href='/live/source-effectiveness'>organisation unique source count</a></p></div></div><p>Live evidence last collected: {escape(str(detail['live_last_collected']))} · live evidence score: {detail['live_evidence_score']} · learned evidence score: {detail['learned_evidence_score']} · Rob score adjustment: {detail['rob_score_adjustment']} · evidence confidence: {detail['evidence_confidence']} · strongest condition: {escape(detail['strongest_condition'])} · strongest capability: {escape(detail['strongest_capability'])}</p></section><section class='card'><h2>Evidence-first score calculation audit</h2><p>Audit arithmetic: live evidence score + learned evidence score + Rob score adjustment − missing evidence penalty = final score. Seeded fallback score is shown only when used.</p><table>{audit_rows}</table></section><section class='card'><h2>Rob score</h2><p><a href='/score/{escape(detail['organisation'].replace(' ', ''))}/rob-score'>Add Rob score</a></p><p>Latest Rob rationale: {escape(detail['rob_score_reason'] or 'No Rob rationale recorded yet.')} This changes the final score by {detail['rob_score_adjustment']} points.</p></section><section class='card'><h2>Score breakdown facets</h2>{facets}</section><section class='card'><h2>Score Trace</h2><table><thead><tr><th>Evidence object</th><th>condition/capability</th><th>facet</th><th>score contribution</th><th>final score</th></tr></thead><tbody>{traces}</tbody></table></section><section class='card'><h2>Top contributing evidence objects</h2><table><thead><tr><th>Evidence</th><th>Source name</th><th>Source type</th><th>URL</th><th>Snippet</th><th>Mapped condition</th><th>Mapped capability</th><th>Confidence</th><th>Evidence quality</th></tr></thead><tbody>{evidence}</tbody></table></section><section class='card'><h2>Missing evidence that would increase confidence</h2><ul>{missing}</ul></section>"""
