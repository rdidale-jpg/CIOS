"""Canonical, producer-consumer contract for Flora Blueprint packages.

This model is the single owner of ``blueprint_manifest.json``.  Flora parses it
at receipt and package producers use :func:`build_manifest`/``model_json_schema``;
the JSON Schema is generated from this model rather than maintained separately.
"""
...
class BlueprintManifest(BaseModel):
    ...
    def unique_paths_and_references(self):
        ...
        if self.final_twin_spine_workbook and self.final_twin_spine_workbook not in {item.path for item in self.files}:
