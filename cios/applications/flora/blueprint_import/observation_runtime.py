...
def build_candidate_observation(obj: SemanticObject, *, observed_at: str | None = None) -> tuple[CandidateObservation | None, str, str]:
    ...
    projection = factual_projection_for_object(obj, family)
    statement, fields = _statement_and_fields(obj, projection)
    if not statement:
...
def _statement_and_fields(obj: SemanticObject, projection=None) -> tuple[str, tuple[str, ...]]:
    ...
    if projection is not None:
        first = next((section for section in projection.sections if section.present), None)
        if first:
            return f"{obj.subject} — {first.label}: {_render_value(first.values)}", ("canonical_factual_projection", first.label)
    return "", ()
