...
def _executive_enterprise_intelligence_html(ent: SemanticEnterprise, twin: SemanticTwin, run_id: str) -> str:
    ...
    signals = "".join(
        f"<li data-source-object-id='{escape(item.source_id)}'><strong>{escape(item.source_type)}:</strong> {escape(item.title)}</li>"
        for item in intelligence.signals) or "<li>No qualifying change or investment signal is supplied.</li>"
    opportunities = "".join(
        f"<li data-business-object-id='{escape(business_object_id(item))}'><a href='/blueprint-import/{escape(run_id)}/explore?collection=opportunities#{escape(item.record_id)}'>{escape(_field(item, 'title') or item.statement or business_object_id(item))}</a>"
        f"<span>{escape(_field(item, 'client_problem', 'customer_problem', 'problem') or 'Customer problem not supplied')}</span></li>"
        for item in intelligence.opportunities) or "<li>No canonically associated Opportunity is supplied.</li>"
    watchpoints = "".join(
        f"<li data-source-object-id='{escape(item.source_id)}'>{escape(item.title)}</li>"
    ...
    return ("<section class='card executive-intelligence' id='executive-intelligence'><h2>Executive Intelligence</h2>"
            ...
            f"<article><h3>Commercial opportunities <span class='pill'>{len(intelligence.opportunities)}</span></h3><ul class='executive-opportunities'>{opportunities}</ul></article>"
