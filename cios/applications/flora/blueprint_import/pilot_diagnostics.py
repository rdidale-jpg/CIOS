...
def field_panel(obj: SemanticObject|None, label: str, expected_paths: Iterable[str], *, rendered: Any="", target: str="", page_field: str="") -> str:
    ...
    <h4>Stage 4 — Canonical owner assessment</h4><p>canonical owner selected: semantic_twin.{escape(obj.kind if obj else 'unavailable')} · owner invoked: {owner_invoked} · assessment model: ExecutiveAssessmentProjection · assessment produced: no · assessment persistence location: not persisted for candidate import · assessment identifier: not supplied. factual candidate content: {'present' if _present(selected[1]) else 'absent'} · owner assessment: not supplied · completeness judgement: not supplied · recommendation eligibility: not established · promotion eligibility: separate human import action. factual display remains inspectable.</p>
