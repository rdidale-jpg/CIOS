...
def _execution_trace_section(package, summary: dict[str, Any], fatal: bool) -> str:
    ...
    governed = package.package_inspection.get("contract_type") == "Governed Industry Twin Package"
    title = "Governed Industry Twin package diagnostics" if governed else "Blueprint import execution trace"
    stages = next((event.get("diagnostic_steps", []) for event in reversed(trace) if event.get("diagnostic_steps")), []) if governed else []
    stage_list = "<h3>Contract processing</h3><ol>{}</ol>".format("".join(f"<li>{escape(str(stage))}</li>" for stage in stages)) if stages else ""
    return "<section class='card'><h2>{title}</h2>{plain}{stages}{trace_table}{deployment}{package}{workbook}{flow}<h3>Owner next action</h3><p>{guidance}</p><p><button type='button' data-diagnostic-trace='{copy}'>Copy diagnostic trace</button> <a download='blueprint-import-trace.json' href='data:application/json,{copy}'>Download diagnostic trace as JSON</a></p></section>".format(title=title, plain=plain, stages=stage_list, trace_table=trace_table, deployment=table("Deployment", deployment), package=table("Package", pkg_rows), workbook=table("Workbook processing", workbook_rows), flow=table("Validation flow", flow_rows), guidance=escape(guidance), copy=escape(copy, quote=True))
