...
def _executive_review(ctx, job, details, counts, proposed, query, message="") -> str:
    ...
    human_state = "Human import decision accepted — promoted" if promotion_state == "Promoted" else ("Human import decision recorded — not promoted" if human_review else "Candidate — awaiting human import decision")
    identity_html = f"""<section class='hero candidate-review-identity' aria-labelledby='candidate-review-title'><p class='pill'>{escape(review_state)}</p><h1 id='candidate-review-title'>{escape(name)}</h1><h2>Import acceptance summary</h2><p><strong>Package:</strong> {escape(package.original_filename)} · <strong>Checksum:</strong> {escape(package.package_sha256)}</p><p><strong>Proposed {escape(twin_type)}</strong> · primary subject: {escape(str(subject))} · governed scope: {escape(str(scope))}</p><p><strong>Geography:</strong> {escape(str(geography))} · <strong>Time horizon:</strong> {escape(str(horizon))}</p><p><strong>Records accepted:</strong> {int(counts.get('Accepted', 0))} · <strong>Rejected:</strong> {int(counts.get('Rejected', 0))} · <strong>Quarantined:</strong> {quarantined}</p><p><strong>Unresolved Unknowns:</strong> {len(unknowns)} · <strong>Contradictions:</strong> {len(contradictions)} · <strong>Association anomalies:</strong> 0 · <strong>Missing canonical subjects:</strong> {1 if unresolved else 0}</p><p><strong>Review status:</strong> {escape(review_state)} · <strong>Promotion status:</strong> {escape(promotion_state)} · <strong>Human import state:</strong> {escape(human_state)} · <strong>Recommendation status:</strong> Not eligible</p><p class='warning'><strong>Human review makes no canonical change.</strong> Promotion remains a separate decision.</p><p><a href='/blueprint-import/{escape(package.import_run_id)}/health'>Inspect residual Research Gaps</a> · <a href='/blueprint-import/{escape(package.import_run_id)}/intelligence'>Return to Inspect</a></p></section>"""
    if not human_review:
