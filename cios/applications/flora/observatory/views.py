...
def organisation_observatory_page(slug: str) -> str:
    ...
    signal_cards = "".join(f"<article class='card'><h3>{escape(s.title)}</h3><p>{escape(s.observation)}</p><p><strong>Supports:</strong> {escape(', '.join(s.supports))}</p><p><strong>Does not support:</strong> {escape(', '.join(s.does_not_support))}</p><p><strong>Confidence:</strong> {s.confidence} · <a href='{escape(s.source_url)}'>source</a> · Evidence {escape(', '.join(s.supporting_evidence_ids))}</p></article>" for s in org.commercial_signals[:5])
