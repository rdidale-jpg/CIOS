...
OBSERVATORY_VERSION = "newton-sprint-15"
...
def _report_metadata(obs, org=None):
    evidence_cutoff = max((e.extraction_timestamp or e.evidence_freshness for e in obs.evidence), default="No evidence")
    ...
    return f"""<section class='card metadata'><h2>Report Metadata</h2><table>
    <tr><th>Report generated timestamp</th><td>{escape(generated)}</td></tr>
    <tr><th>Evidence cut-off timestamp</th><td>{escape(str(evidence_cutoff))}</td></tr>
...
def organisation_observatory_page(slug: str) -> str:
    ...
    body = _report_metadata(obs, org) + f"""<section class='hero'><h1>{escape(org.organisation)} Transformation Genome</h1><h2>Commercial Intelligence Briefing</h2><p>{escape(org.sector)} · Conversation level: <strong>{escape(case.conversation_level)}</strong> · Confidence {case.confidence}</p><p>{escape(case.conversation_elevation_reason)}</p></section>
