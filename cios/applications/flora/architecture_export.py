...
def github_api_base_url() -> str:
    return os.getenv("FLORA_GITHUB_API_BASE_URL", "https://api.github.com").rstrip("/")
...
def _github_failure_reason(endpoint: str, status_code: int) -> str:
    if status_code in {401, 403}:
        if "/dispatches" in endpoint:
            return "Workflow dispatch forbidden"
        if "/artifacts" in endpoint:
            return "Artifact read forbidden"
        if "/actions/" in endpoint:
            return "Insufficient Actions permission"
        return "Credential invalid" if status_code == 401 else "Repository inaccessible"
    if status_code == 404:
        if "/actions/workflows/" in endpoint:
            return "Workflow not found"
        return "Repository inaccessible"
    return f"GitHub API failed: {status_code}"
...
def _github(method: str, endpoint: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    req = urllib.request.Request(f"{github_api_base_url()}{endpoint}", data=data, method=method)
    ...
    try:
        ...
        raise RuntimeError(_github_failure_reason(endpoint, exc.code)) from exc


def _local_workflow_found() -> bool:
    workflow = configured_workflow()
    return Path(workflow).exists() or Path(".github/workflows").joinpath(workflow).exists()


def _manifest_found() -> bool:
    return Path("FLORA_ARCHITECTURE_DOWNLOAD_MANIFEST.json").exists()


def _remote_integration_reason(repo: str) -> str:
    workflow = configured_workflow()
    branch = default_branch()
    _github("GET", f"/repos/{repo}")
    _github("GET", f"/repos/{repo}/actions/workflows/{workflow}")
    try:
        _github("GET", f"/repos/{repo}/contents/FLORA_ARCHITECTURE_DOWNLOAD_MANIFEST.json?ref={branch}")
    except RuntimeError as exc:
        if "Repository inaccessible" in str(exc):
            raise RuntimeError("Manifest not found") from exc
        raise
    _github("GET", f"/repos/{repo}/actions/workflows/{workflow}/runs?per_page=1")
    try:
        _github("POST", f"/repos/{repo}/actions/workflows/{workflow}/dispatches", {"ref": "__flora_permission_probe__", "inputs": {}})
    except RuntimeError as exc:
        if "Workflow dispatch forbidden" in str(exc) or "Insufficient Actions permission" in str(exc):
            raise
        if "GitHub API failed: 422" in str(exc):
            pass
    _github("GET", f"/repos/{repo}/actions/artifacts?per_page=1")
    return "Ready"
...
def github_integration_status() -> dict[str, Any]:
    ...
    workflow_available = bool(configured_workflow())
    workflow_found = _local_workflow_found()
    manifest_found = _manifest_found()
    reasons = []
    if not repo:
    ...
    if repo and credential and workflow_found and manifest_found:
        try:
            _remote_integration_reason(repo)
            status = "Ready"
        except RuntimeError as exc:
...
def dispatch_export(headers: Any, *, requested_ref: str = "", export_profile: str = DEFAULT_PROFILE, publish_mode: str = "artifact", include_audit_reports: bool = True, explicit_release_confirmation: bool = False) -> ExportAuditRecord:
    ...
    _github("POST", f"/repos/{configured_repository()}/actions/workflows/{configured_workflow()}/dispatches", {"ref": default_branch(), "inputs": {"git_ref": ref, "export_profile": export_profile, "publish_mode": publish_mode, "include_audit_reports": str(include_audit_reports).lower()}})
...
def architecture_export_page(headers: Any) -> tuple[str, int]:
    ...
    empty = "" if ready_to_generate else f"<section class='card action'><h2>Architecture Export unavailable</h2><p><strong>Reason:</strong> {escape(integration['reason'])}</p><p><a href='/settings/general'>View configuration</a> · <a href='/settings/architecture-export'>Retry status</a> · <a href='https://github.com/{escape(configured_repository())}/actions/workflows/{configured_workflow()}'>Open workflow run</a></p></section>"
    ...
    body = f"""<section class='hero'><h1>Architecture Export</h1><p>Generate and download a governed architecture baseline package from the configured GitHub repository.</p></section><section class='card'><table>{table}</table></section>{empty}<section class='card action'>{generate}<p><a href='/settings/architecture-export'>Retry status</a></p>{download}<p><a href='/settings/architecture-export/manifest'>View manifest</a> · <a href='/settings/architecture-export/exclusions'>View exclusions</a> · <a href='https://github.com/{escape(configured_repository())}/actions/workflows/{configured_workflow()}'>View workflow run</a></p>{expired}</section>"""
