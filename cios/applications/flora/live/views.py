...
const terminal = new Set(['completed','completed_with_exceptions','completed_with_no_accepted_intelligence','failed','interrupted','completed successfully','Completed with no accepted intelligence']);
...
def rejected_claims_page(run_id: str | None = None) -> str:
    ...
    rows = "".join(
        "<tr>"
        f"<td>{escape(str(c.get('claim_text') or c.get('proposed_atomic_statement') or ''))}</td>"
        f"<td>{escape(str(c.get('source_id') or ''))}</td>"
        f"<td>{escape(str(c.get('page') or c.get('document_page') or ''))}</td>"
        f"<td>{escape(str(c.get('failed_validation') or c.get('validation_rule') or ''))}</td>"
        f"<td>{escape(str(c.get('rejection_reason') or ''))}</td>"
        f"<td>{escape(str(c.get('intended_domain') or c.get('model_domain') or ''))}</td>"
        f"<td>{escape(str(c.get('problem_stage') or 'validation'))}</td>"
        "</tr>"
        for c in claims
    )
...
def evidence_page() -> str:
    ...
    unsupported_table_rows = unsupported_rows or '<tr><td colspan="4">None recorded.</td></tr>'
