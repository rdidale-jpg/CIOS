...
def _evidence_row(e: dict[str, Any]) -> str:
    ...
    return f"<tr><td>{org}</td><td>{escape(str(e.get('source_name','')))}</td><td>{link_or_label(e.get('source_url'))}</td><td>{link_or_label(e.get('source_url'), 'display URL')}</td><td>{escape(str(e.get('evidence_type','')))}</td><td>{escape(str(e.get('cleaned_observation') or e.get('snippet','')))}</td><td>{escape(str(e.get('commercial_condition','')))}</td><td>{escape(str(e.get('likely_capability','')))}</td><td>{escape(str(e.get('relevance_level','')))}</td><td>{escape(str(e.get('confidence','')))}</td><td>{escape(str(e.get('evidence_quality_band') or e.get('overall_evidence_quality','')))}</td><td>{escape(str(e.get('extraction_timestamp','')))}</td><td>{fb}</td></tr>"
