...
from cios.applications.flora.memory.source_contracts import classify_foundation_eligibility
...
def collect(organisation: str | None = None, *, profile_id: str | None = None, collection_mode: str | None = None, run_id: str | None = None, passes: list[str] | None = None) -> dict[str, Any]:
    ...
    for item in new_evidence:
        item.update(classify_foundation_eligibility(item))
    ...
    obs_created = len([r for r in memory_results if r.action in {"created", "unknown_created", "contradiction_recorded"}])
    ...
    successful_intelligence = (obs_created + obs_corroborated + attrs_created) >= 1
    candidate_failures = bool(rejected_claims or source_failures)
    foundation_eligible_count = len([e for e in new_evidence if e.get("foundation_eligible")])
    context_signal_count = len([e for e in new_evidence if not e.get("foundation_eligible")])
    result_state = "completed_with_no_foundation_facts" if len(new_evidence) and not foundation_eligible_count else "completed_with_no_accepted_intelligence"
    if successful_intelligence:
        result_state = "completed_with_partial_intelligence" if candidate_failures else "completed"
    manifest = {"run_id": run_id, "application_revision": application_revision(), "canonical_enterprise_id": scoped_enterprise_id, "enterprise_display_name": scoped_display_name, "profile_id": profile_id, "started_at": collection_started_at, "completed_at": completed_at, "collection_mode": mode, "passes": selected_passes, "sources_planned": [s.source_id for s in sources], "sources_attempted": [d["source_id"] for d in diagnostics], "sources_retrieved": [d["source_id"] for d in diagnostics if d["status"] != "failed"], "sources_failed": [d["source_id"] for d in source_failures], "evidence_candidates": evidence_candidates, "evidence_accepted": len(new_evidence), "foundation_eligible_evidence": foundation_eligible_count, "context_signal_only_evidence": context_signal_count, "evidence_rejected": evidence_rejected, "evidence_downgraded": evidence_downgraded, "evidence_context_only": sum(int(d.get("context_only_count") or 0) for d in diagnostics), "evidence_duplicate": duplicate_count, "evidence_corroborated": obs_corroborated, "evidence_extraction_failed": 0, "factual_claims_extracted": sum(r.factual_claims_extracted for r in processing_reports), "factual_claims_accepted": sum(r.factual_claims_accepted for r in processing_reports), "factual_claims_rejected": sum(r.factual_claims_rejected for r in processing_reports), "factual_claims_duplicate": sum(r.factual_claims_duplicate for r in processing_reports), "factual_claims_corroborated": sum(r.factual_claims_corroborated for r in processing_reports), "rejected_claims": rejected_claims, "accepted_evidence_diagnostics": accepted_diagnostics, "decomposition_diagnostics": decomposition_diagnostics, "documents_retrieved": sum(int(d.get("documents_retrieved") or 0) for d in diagnostics), "pdfs_parsed": sum(int(d.get("pdfs_parsed") or 0) for d in diagnostics), "pages_extracted": sum(int(d.get("pages_extracted") or 0) for d in diagnostics), "tables_detected": sum(int(d.get("tables_detected") or 0) for d in diagnostics), "golden_facts_matched": 0, "observations_created": obs_created, "observations_corroborated": obs_corroborated, "observations_rejected": len(rejected_claims), "model_attributes_created": attrs_created, "model_attributes_changed": len([r for r in memory_results if r.action == "updated"]), "model_attributes_reconfirmed": obs_corroborated, "unknowns_created": unknowns_created, "contradictions_created": contradictions_created, "result_state": result_state, "result_reason": "accepted Evidence was context or Signal-only." if result_state == "completed_with_no_foundation_facts" else "", "errors": [d for d in diagnostics if d.get("status") == "failed"], "warnings": [f"{len(rejected_claims)} factual claims rejected"] if rejected_claims else []}
