...
from .models import *
from .reasoning import DeterministicDevelopmentAdapter, ReasoningAdapter
...
def retrieve(run_id:str, plan:ContextPlan)->RetrievalSet:
    ...
    obs=[{'observation_id':o,'statement':('App-first but not app-only distribution / assisted-access lineage' if o!='BK-OBS-047' else 'Branches operate as both cost burdens and trust assets depending on participant type'),'relevance':'High','confidence':'Medium','evidence_refs':['EK-BANK-RHYP-001','BK-GOV-SSN-VAL-001'],'freshness':'Unknown'} for o in plan.observation_ids]
    mech=[{'mechanism_id':m,'relationship_to_observations':plan.observation_ids,'relevance':'High','confidence':'Medium','authority_asset_id':'BK-CMP-002'} for m in plan.mechanism_ids]
...
def run_pipeline(output_dir:Path=OUT, adapter:ReasoningAdapter|None=None, extra_asset_ids:list[str]|None=None)->PipelineRun:
    adapter=adapter or DeterministicDevelopmentAdapter(); run_id=_rid(); audit=[]; stages={}
    ...
    mech=MechanismAssessment(object_id=run_id+'-mechanisms',run_id=run_id,source_asset_ids=['BK-CMP-002','BK-REF-001'],relationship_paths=['BRH-003->BM-*->BK-OBS-*'],confidence='Medium',unknowns=obs.unknowns,contradictions=obs.contradictions,mechanisms=retrieval.mechanisms,alternatives=['Physical access may remain primarily a cost optimisation lever'],applicability_constraints=['Participant type changes mechanism behaviour']); stages['mechanism_assessment']=mech
    ...
    for n,k in zip(STAGES,stages): audit.append(AuditEvent(stage=n,status=stages[k].status,message='stage emitted schema-valid object',object_id=stages[k].object_id))
    validation=validate(run_id,q,stages,retrieval.assets)
    run=PipelineRun(run_id=run_id,question=q,stages={k:v.model_dump(mode='json') for k,v in stages.items()},audit_events=audit,validation=validation,telemetry={'adapter_model':adapter.model_name,'instruction_version':adapter.instruction_version,'stage_count':len(stages)})
...
def validate(run_id,q,stages,assets):
    ...
    ok &= gate('runtime objects not authoritative/governed', all(v.authority not in ['governed_source'] for v in stages.values()))
