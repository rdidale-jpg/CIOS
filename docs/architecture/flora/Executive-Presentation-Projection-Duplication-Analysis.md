# Executive Presentation Projection Duplication Analysis
...
## Distinct projections/read models

| Projection | Owner | Input | Output/consumers | Coverage | Drift risk |
|---|---|---|---|---|---|
| CandidateRecord payload | `candidates.py`/validator | adapter payload | staging/review/inspect | all accepted records | high when selectors change |
| SemanticTwin | `semantic_twin.py` | candidates | workspace, aspects, diagnostics | major families | high: implementation schema not fixture-declared |
| Canonical Factual Projection | `canonical_factual_projection.py` | SemanticTwin | factual candidate presentation | industry, enterprise, participant, programme, opportunity, reinvention | medium: should be page/diagnostics/gap source |
| Intelligence owner projection | `intelligence_projection.py` | owner outputs/readiness | readiness/research gaps | owner-assessed dimensions | high if it ignores factual projection presence |
| Page-specific view models | `executive_workspace.py` | semantic + readiness + factual fragments | HTML pages | family-dependent | high; source of raw dict/list display |
| Advanced diagnostics | `executive_workspace.py`, `pilot_diagnostics.py` | semantic, validation summary, observation pipeline | diagnostic pages | all pages partially | high; can contradict page rendering |
| Research Gap actions | `executive_workspace.py`, `research_requirements.py` | assessment state/deficiencies | research actions/brief | gaps by aspect | high; can request visible facts |
