...
The Enterprise Intelligence architecture defines evidence-governed interpretation over accepted Enterprise Twin retrieval. For imported candidates, the Canonical Factual Projection (CFP) is the shared, read-only factual owner and the executive workspace is its presentation consumer. The semantic Twin relationship query owns Enterprise associations. Recommendation eligibility and promotion remain separate governance concerns.

This increment distinguishes architectural intent from runtime capability: Enterprise Economics and Leadership / Governance remain unsupported independent TEL-001 dimensions. The current programme state is candidate intelligence awaiting a human import decision; this derivative neither assesses nor promotes it.

## Canonical owners and reused capability

| Executive element | Canonical runtime owner consumed |
| --- | --- |
| Situation | `CanonicalFactualProjection.enterprise_synthesis` and its qualifying propositions |
...
## Source dimensions and object paths
...
## Preservation and protected boundaries

Evidence references remain attached to every selected signal and in panel explainability. Unknowns and Contradictions are copied only into the immutable derivative and remain unchanged in canonical state. Human import state remains **Candidate — awaiting human import decision**. Domain remains **Not established** where explicit semantic domain is absent. Enterprise Economics and Leadership / Governance remain reported as architectural intent—not implemented. Recommendation status remains not eligible and recommendation governance is unchanged.

BT Material Pressures remains a potential canonical semantic coverage issue: relevant pressure language exists elsewhere while no dedicated qualifying pressure fact is supplied. This increment does not copy that language into Material Pressures.

## Runtime validation and merge gate

The runtime acceptance executes implementation, consumption and rendering against all six real Enterprise routes. Protected fixture checks retain 308 Relationships, 13 Programmes and 17 Opportunities; canonical Enterprise association sets are unchanged; and the pre-existing Advanced Inspection factual reconciliation remains PASS. Current Change declares that no fresh import is required.

The merge gate is safe only while these checks remain green and no independent persistence or recommendation behavior is introduced.
