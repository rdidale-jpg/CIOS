...
## Slice 2D — BT Commercial Digital Twin product workspace

Slice 2D adds the first Flora Commercial Digital Twin product surface for BT Group at `/digital-twins` and `/digital-twins/bt-group-plc`. The page is a runtime view over durable Enterprise Model state, standard Financial Intelligence run records, approved manifest availability and persisted Slice 2C candidate lineage. It is not canonical storage and introduces no separate Digital Twin database, candidate store, financial-fact store, copied evidence repository or copied source-document store.

The BT workspace separates trusted and candidate state. The “What the Twin knows” section reads only from the Enterprise Model repository and shows an honest empty state when no FY26 financial facts have completed verification and acceptance. The “New findings awaiting verification” section reads only from the latest relevant unified Financial Intelligence run and labels candidates as verification pending. Source-backed drill-down uses the persisted candidate fields and source receipt fields: metric label, displayed value, period, period dates, group scope, statutory/actual basis, source title, source authority, page/table/row/column locator and supporting excerpt. The view does not reacquire the source and does not infer missing source locations.

The BT action labelled “Search for new information” invokes the existing explicit `dual_speed_financial_intelligence` mode for the FY26 approved BT source. The normal Financial Intelligence default remains `structured_standard_financials`. The action persists only the existing standard run record. AI calls remain zero, provider cost remains zero, and canonical writes remain zero: Evidence, Observations and Enterprise Model attributes are unchanged by source-backed candidate search.

The workspace renders successful, partial and unavailable outcomes in business language and preserves Unknowns. A successful source-backed run shows revenue, operating profit and profit before tax as new findings outside the trusted Twin. Partial results show available findings and unresolved metrics without zero-valued placeholders. Unavailable source results confirm that no fixture or seeded values were substituted and that the trusted Twin was unchanged. Research history lists the five most recent relevant standard Financial Intelligence runs and links to the existing detailed result view; opening history does not reacquire the source.

User acceptance journey: open Flora, select Digital Twins, open BT Group, press “Search for new information”, read the displayed BT Digital Twin result and expand one financial finding to inspect source lineage. Live deployed access to the approved BT source remains an explicit Unknown until the deployed product action succeeds after merge and deployment.

## BT FY26 live PDF parser compatibility update

Rapid Financial Intelligence now uses one page-aware PDF parsing boundary for both Slice 2A source validation and Slice 2B candidate extraction. The supported runtime parser is PyMuPDF, declared in production dependencies. The boundary opens file-backed PDFs, returns parser implementation/version, page count, page-numbered text, bounded page-level extraction errors and document parse status, and does not use OCR, AI or external providers.

The hosted BT FY26 failure was traced to the previous Slice 2A validation parser: it used a narrow embedded-text stream extractor in `live.documents.parse_pdf_document`, while Slice 2B used PyMuPDF. Production validation could therefore reject a valid compressed/multi-page official PDF before candidate extraction. The new shared boundary removes the split definition of "parsed" and preserves page locators for the three unverified candidate findings. Bounded diagnostics are persisted with the run; full PDF bytes, temporary file paths, full extracted text, stack traces and credentials are not persisted.

BT Digital Twin research remains non-canonical: AI calls are zero, provider cost is zero, Evidence/Observation/Enterprise Model writes are not made, and candidate findings remain verification-pending until later structured verification and canonical acceptance.
