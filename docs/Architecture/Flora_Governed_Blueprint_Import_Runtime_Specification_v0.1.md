...
## Sprint 1 PR4 runtime note — governed canonical promotion

Flora now implements the bounded PR4 canonical-promotion boundary after PR3 dry-run planning. Promotion is not automatic: an attributable approval record must name the approved dry-run plan, import run, package reference and checksum, approver identity, approval timestamp, rationale, approved effects, approved expected mutation count, accepted unresolved warnings and invalidation condition. Execution refuses an approval if the plan content, review decision set, mapping set or package checksum has changed, if blocking conflicts remain, if the actor lacks promotion permission or if the approval has been superseded.

The safe canonical promotion class set is deliberately narrow. PR4 can create or update canonical `evidence` records through the existing Flora `EvidenceRepository`, and can create canonical `observation` records through the existing `ObservationRepository` only where the candidate payload already satisfies the existing Observation contract. Source, Unknown, Contradiction, Human-Supplied Knowledge as a distinct store, Enterprise Model candidate updates, entity/relationship graph objects, refresh triggers and publication references remain deferred unless represented by an already supported Evidence or Observation contract. Analytical projections, including Pain Points, Burning Platforms and Transformation Pressure views, remain non-canonical and are skipped by promotion.

Only approved `create` and `update` effects mutate canonical state. Mapping to an existing record, unchanged, duplicate, rejected, deferred, quarantined, unsupported, unresolved, projection and blocked-conflict effects are recorded in the promotion result without canonical mutation. Existing records are not overwritten merely because an external ID matches; an update must be present in the approved dry-run plan and the prior canonical version is retained in the Blueprint import lineage attached to the write.

Each created or updated canonical record receives inspectable Blueprint import lineage, including package reference/version/checksum, import-run ID, candidate ID, review-decision ID, mapping ID when present, dry-run plan ID, approval ID, original external ID, source file, source location, source fingerprint, approving actor, executing actor and promotion timestamp. Promotion execution records are append-only events in the Blueprint import ledger and durable JSON result files under `blueprint_import/promotion/executions/{import_run_id}/`.

Promotion execution is idempotent for the same approved plan and approval. A repeated successful execution returns an explicit repeat/no-change result instead of creating duplicate canonical records or mappings. Failure handling uses file-level canonical backups for the existing JSONL stores touched by PR4 and restores those backups before recording a failed append-only promotion result; this avoids unexplained partial canonical state without introducing a new persistence framework.

Reversal remains deferred. PR4 records sufficient execution and lineage data for future reversal planning, but it does not implement reversal execution because the audit places full reversal support behind owner-specific canonical services that do not yet exist for all future object classes.
