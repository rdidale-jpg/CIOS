...
def test_compiler_parses_registry_rows() -> None:
    ...
    assert {"AP-001", "AP-002", "RP-001", "RP-002", "EIF-001", "EOD-001", "EU-001", "OT-001", "EI-014", "EI-015", "EI-017", "FP-010", "FP-013", "OPI-001", "RTP-001", "ADR-023", "VAL-ROADMAP-001"}.issubset(ids)
...
def test_architecture_authority_profile_includes_only_accepted_authority() -> None:
    ...
    assert {"EIF-001", "EOD-001", "EU-001", "OT-001", "EI-014", "EI-015", "EI-017", "FP-010", "FP-013", "OPI-001", "RTP-001", "ADR-023", "VAL-ROADMAP-001"}.issubset(excluded_ids)
...
def test_review_context_preserves_review_and_proposed_statuses() -> None:
    ...
    assert status_by_id == {"EIF-001": "Review", "EOD-001": "Review", "EU-001": "Review", "OT-001": "Review", "EI-014": "Review", "EI-015": "Review", "EI-017": "Review", "FP-010": "Review", "IT-001": "Review", "OPI-001": "Review", "RTP-001": "Review", "ADR-023": "Proposed", "VAL-ROADMAP-001": "Review", "IC-001": "Review", "FP-012": "Review", "FP-013": "Proposed", "ADR-025": "Proposed", "FEIR-001": "Proposed", "EIRP-001": "Proposed", "UX-001": "Proposed"}
...
def test_fp010_review_material_is_visible_only_in_review_context() -> None:
    review_ids = {document.document_id for document in compile_architecture_profile("review-context", ROOT).included_documents}
    assert "FP-010" in review_ids

    for profile in ("architecture-authority", "researcher-pack", "assurance-pack", "reviewer-pack"):
        production_ids = {document.document_id for document in compile_architecture_profile(profile, ROOT).included_documents}
        assert "FP-010" not in production_ids
...
def test_ei017_review_material_is_visible_only_in_review_context() -> None:
    review_ids = {document.document_id for document in compile_architecture_profile("review-context", ROOT).included_documents}
    assert "EI-017" in review_ids

    for profile in ("architecture-authority", "researcher-pack", "assurance-pack", "reviewer-pack"):
        production_ids = {document.document_id for document in compile_architecture_profile(profile, ROOT).included_documents}
        assert "EI-017" not in production_ids
