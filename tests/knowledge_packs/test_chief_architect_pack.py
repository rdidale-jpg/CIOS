...
def test_manifest_completeness_sources_and_shared_authorities():
    ...
    required={'CA-001','FP-010','FP-012','FP-009','ADR-016','ADR-014','ADR-024','REF-001','PRINCIPLES-001','CURRENT-PROGRAMME-STATE','FEIR-001','EIRP-001','EI-001','EI-002','EI-003','EI-012','EIF-001','KPS-001','CA-OG-001','CA-TEMPLATE-001','CA-SOURCE-MAP'}
    assert required <= ids
...
def test_runtime_programme_sources_are_not_absent_or_placeholder_like():
    ds=docs(); by={d['document_id']:d for d in ds}
    for did in ['CURRENT-PROGRAMME-STATE','FEIR-001','EIRP-001','ADR-014','ADR-024']:
...
def test_deterministic_build_zip_checksum_index_pack_state_and_completeness_matrix():
    ...
    for criterion in ['Programme-state baseline','Runtime-baseline authority','Operating guidance','Templates','Source map','Core Enterprise Intelligence authorities','Placeholder rejection']:
